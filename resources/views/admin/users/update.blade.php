            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="mb-3">
                        ...
                        <select
                            ...
                            @foreach($roles as $role)
                                <option value="{{ $role->id }}" {{ (old("role", $user->roles->first()->id??null) == $role->id ? "selected":null) }}>{{ $role->name }}</option>
                            @endforeach
                        </select>
                        @if($errors->has('role'))
                    ...
                    <div class="alt_button_holder">
                        <button type="button" class="btn btn-danger delete_btn" data-route="{{ route('admin.users.destroy', $user->id) }}">Sil</button>
